...
#define K 2.5
#define ref_sign_freq 1 //Hz
#define MAX_REF_SIGNAL 3048 //over 4096
#define MIN_REF_SIGNAL 1024
...
int count = 0;
int ref_signal = 0;
int state = 0; //0 is up, 1 is down
int feedback_signal = 0;  // variable to store the value read
int output = 0;

void SetTheVoltage(){  
 	feedback_signal = analogRead(feedBackPin);
 	count++;

 	if (state == 0){
 		ref_signal = MAX_REF_SIGNAL;
 		if (count > 1000){
 			state = 1;
 			count = 0;
 		}
 		...
 		ref_signal = MIN_REF_SIGNAL;
 		if (count > 1000){
 			state = 0;
 			count = 0;
 		}
 	}
 	output = PID_controller(ref_signal, feedback_signal);  
}
...
int PID_controller(int ref_signal, int feedback_signal){
	// ref_signal & output signal (DAC) range from 0 to 4096, 
	// feedback signal (ADC) range from 0 to 1024
	int error = ref_signal - feedback_signal*4; 
	int output = Kp*error;
...
void loop(void) {
	...
	Serial.print(feedback_signal*4);
	Serial.print("\t");
	Serial.println(ref_signal);
